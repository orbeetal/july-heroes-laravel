<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between gap-2 items-center">
            <a
                href="{{ route('dashboard.murderers.index') }}"
                class="cursor-pointer text-gray-600 hover:text-gray-900"
            >
                &larr; Back to List
            </a>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __("Murderer Edit Form") }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="container mx-auto sm:px-6 lg:px-8">
            <form
                method="POST"
                action="{{ route('dashboard.murderers.update', $murderer->id) }}"
                class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4 md:p-8 grid gap-4"
                enctype="multipart/form-data"
            >
                @csrf @method('PUT') @include('admin.murderers.form')
